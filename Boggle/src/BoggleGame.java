...
public class BoggleGame extends JFrame {
	...
	public BoggleGame(int rows, int cols) throws FileNotFoundException {
		...
		Scanner in = null;
		try {
			in = new Scanner(new File(PATH + "dictionary-shakespeare.txt"));
		...
		// all words in shakespeare
		try {
			in = new Scanner(new File(PATH + "dictionary-nursery.txt"));
		} catch(FileNotFoundException f) {
		}
		...
		try {
			in = new Scanner(new File(PATH + "dictionary-common.txt"));
		} catch(FileNotFoundException f) {
		}
		...
		try {
			in = new Scanner(new File(PATH + "dictionary-algs4.txt"));
		} catch(FileNotFoundException f) {
		}
		...
		// create the Boggle solver with the big dictionary
	...
	private void newGame() {
		...
		if(gameDifficulty == NURSERY) {
			...
			for(String word : validWords)
				if(StdRandom.uniform(4) != 0) //25% chance of using word if it can be found in yawl?
					opponentFoundWords.add(word);
		}
